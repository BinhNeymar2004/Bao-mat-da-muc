...
class EncryptionApp:
    def __init__(self, master):
        ...
        self.main_canvas = tk.Canvas(self.master, bg=self.colors["light"], highlightthickness=0)
        ...
        self.wave_canvas = tk.Canvas(logo_frame, bg=self.colors["primary"], highlightthickness=0, height=80)
    ...
    def setup_algorithm_info_tab(self):
        ...
        for row_idx, algo_row in enumerate(algorithm_data):
            ...
            for col_idx, cell_data in enumerate(algo_row):
                cell_label = ttk.Label(row_frame, text=cell_data, 
    ...
    def select_file(self):
            ...
            highlight_style.configure("Highlight.TFrame", background=self.colors["highlight"])
            self.file_info_frame.configure(style="Highlight.TFrame")
    ...
    def finish_otp_generation(self, success, purpose):
        ...
        if success:
            if purpose == "mã hóa":
                ...
                # Tạo style highlight tạm thời
                highlight_style = ttk.Style()
                highlight_style.configure("Highlight.TFrame", background=self.colors["highlight"])
                self.encrypt_otp_frame.configure(style="Highlight.TFrame")
                ...
                highlight_style = ttk.Style()
                highlight_style.configure("Highlight.TFrame", background=self.colors["highlight"])
                self.decrypt_otp_frame.configure(style="Highlight.TFrame")
    ...
    def encrypt_file(self):
        ...
        if not self.encryption_otp:
            ...
            original_bg = self.encrypt_otp_button.cget("background")
            self.encrypt_otp_button.configure(style="Danger.TButton")
        ...
        if entered_otp != self.encryption_otp:
            ...
            # Tạo style danger tạm thời
            danger_style = ttk.Style()
            danger_style.configure("Danger.TFrame", background=self.colors["danger"])
            self.encrypt_otp_frame.configure(style="Danger.TFrame")
            self.master.after(500, lambda: self.encrypt_otp_frame.configure(style="TFrame"))
            return
    ...
    def finish_encryption(self, success, output_file=None, algorithm_name=None, selected_algorithms=None, error_message=None):
        ...
        if success:
            ...
            highlight_style = ttk.Style()
            highlight_style.configure("Highlight.TFrame", background=self.colors["highlight"])
            self.hash_info_frame.configure(style="Highlight.TFrame")
    ...
    def decrypt_file(self):
        ...
        if not self.decryption_otp:
            ...
            # Highlight nút tạo OTP
            self.decrypt_otp_button.configure(style="Danger.TButton")
        ...
        if entered_otp != self.decryption_otp:
            ...
            danger_style = ttk.Style()
            danger_style.configure("Danger.TFrame", background=self.colors["danger"])
            self.decrypt_otp_frame.configure(style="Danger.TFrame")
            self.master.after(500, lambda: self.decrypt_otp_frame.configure(style="TFrame"))
            return
    ...
    def finish_decryption(self, success, output_file=None, algorithm_name=None, 
        ...
        if success:
            ...
            if hash_match:
                ...
                success_style.configure("Success.TFrame", background=self.colors["success"])
                self.hash_info_frame.configure(style="Success.TFrame")
                ...
                danger_style = ttk.Style()
                danger_style.configure("Danger.TFrame", background=self.colors["danger"])
                self.hash_info_frame.configure(style="Danger.TFrame")
                self.master.after(800, lambda: self.hash_info_frame.configure(style="Card.TFrame"))
            ...
            if is_image_file(output_file):
                ...
                self.view_image_button.configure(style="Warning.TButton")
